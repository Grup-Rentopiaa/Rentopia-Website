        ...
        <div className="max-w-6xl mx-auto px-6 py-8">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
            ...
            <div className="relative group cursor-pointer flex-shrink-0"
              ...
              {user?.avatarB64 ? (
                <img src={user.avatarB64} className="w-24 h-24 rounded-2xl object-cover"
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              {user?.city && (
                <p className="text-sm mt-1" style={{ color: "#7B6AAA" }}>📍 {user.city}</p>
              )}
              {user?.description && (
                <p className="text-sm mt-2 max-w-md leading-relaxed" style={{ color: "#7B6AAA" }}>
                  {user.description}
                </p>
              )}
              <button onClick={() => setShowEdit(p => !p)}
                className="rp-btn-outline text-sm px-4 py-2 mt-4">
            ...
            <div className="flex rounded-2xl overflow-hidden"
              ...
              {[
                ...
                <div key={s.label} className="flex items-stretch">
                  <StatBox {...s} />
                  {i < arr.length - 1 && (
                    <div className="w-px my-3" style={{ background: "#E8DCFF" }} />
                  )}
                </div>
              ))}
            </div>
          ...
          <section>
            ...
            <div className="mb-4">
              <input value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Cari barang..." className="rp-input text-sm" />
            </div>
            {iLoad ? (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {Array(4).fill(0).map((_, i) => (
                  <div key={i} className="rp-skeleton h-40 rounded-2xl" />
                ))}
              </div>
              ...
              <div className="rp-card py-16 text-center">
                ...
                <button onClick={() => navigate('/upload')} className="rp-btn-primary mt-4 text-sm">
                  Upload Produk
                </button>
              </div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {filteredItems.map(item => (
                  <div key={item.id} className="rp-product-card cursor-pointer"
                    ...
                    <div className="p-3">
                      <h3 className="text-sm font-bold line-clamp-1" style={{ color: "#3D2F6B" }}>
                        {item.title}
                      </h3>
                      ...
                      <span className="rp-badge rp-badge-mint mt-1.5 text-[10px]">
          ...
          <section>
            ...
            {rLoad ? (
              ...
              <div className="rp-card py-16 text-center">
                ...
                <button onClick={() => navigate('/home')} className="rp-btn-primary mt-4 text-sm">
                  Cari Produk
                </button>
              </div>
            ) : (
                ...
                {rentals.map(r => (
                  <div key={r.id} className="rp-card p-4 flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-sm line-clamp-1" style={{ color: "#3D2F6B" }}>
                        {r.title}
                      </h3>
                      <p className="text-xs mt-0.5" style={{ color: "#9B87D9" }}>
                        {formatPrice(r.price)}/hari
                      </p>
